...
- Fix blank response bodies: `ResponseBody` now serializes with an internal `type` tag to match the frontend contract, so text/binary bodies display correctly again (previously `status`/headers showed but body was always empty); backward-compatible with already-stored response data

## [1.2.1] - 2026-09-03

### 🐛 Bugfix

- Fix network-diagnostic and proxy probe crashing (aborting) for HTTPS targets: install the `ring` rustls CryptoProvider so the direct-rustls TLS handshake no longer panics at runtime (release `panic=abort` caused a silent crash / 闪退)
...
- New **Ink & Signal** instrument-console theme: graphite canvas + blueprint grid + phosphor-cyan signal accent
- Self-hosted variable fonts (Saira / Chivo / JetBrains Mono), offline-first
- Request / Response upgraded to recessed instrument panels with an oscilloscope trace while sending

### 🛠️ Stability

- Harden production panic paths: SQLite mutex poisoning recovery, `session.rs` error propagation
