...
pub async fn diagnose_network(
    ...
    let settings = state.services.settings.get()?;
    let skip_cert_verify = !settings.verify_ssl;
    Ok(diagnose(&target, skip_cert_verify).await)
}
...
pub async fn diagnose_proxy_network(
    ...
    let settings = state.services.settings.get()?;
    let skip_cert_verify = !settings.verify_ssl;
    Ok(diagnose_proxy(&target, &proxy, skip_cert_verify).await)
}
