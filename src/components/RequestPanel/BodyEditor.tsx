...
import { tryPretty } from '@/lib/formatter';
...
export function BodyEditor({ value, onChange }: { value: Body; onChange: (b: Body) => void }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {value.type === 'none' && (
        <div className="text-center py-6 text-xs text-gray-400 dark:text-gray-500">请求体为空</div>
      )}
      ...
      {value.type === 'raw' && (
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                const lang: PrettyLang = value.content_type.includes('json')
                  ? 'json'
                  : value.content_type.includes('xml')
                    ? 'xml'
                    : value.content_type.includes('html')
                      ? 'html'
                      : 'text';
                const r = tryPretty(value.content, lang);
                if (r.ok) onChange({ ...value, content: r.text });
              }}
              className="text-[10px] text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300"
            >
              {(() => {
                if (value.content_type.includes('json')) return '美化 JSON';
                if (value.content_type.includes('xml')) return '美化 XML';
                if (value.content_type.includes('html')) return '美化 HTML';
                return '美化';
              })()}
            </button>
          </div>
          <CodeEditor
            ...
            language={
              value.content_type.includes('json')
                ? 'json'
                : value.content_type.includes('xml')
                  ? 'xml'
                  : value.content_type.includes('html')
