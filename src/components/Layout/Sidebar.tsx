...
import { Star, History, FolderOpen, Upload, Plus, Trash2, ChevronRight, ChevronDown } from 'lucide-react';
...
export function Sidebar() {
  ...
  const handleCreateCollection = async (name: string, description: string) => {
    try {
      const c: Collection = { id: nanoid(), name, description, request_ids: [], created_at: Date.now(), updated_at: Date.now() };
      await tauri.saveCollection(c);
      await loadCollections();
      toast.success('集合已创建');
    } catch (e: any) { toast.error('创建集合失败: ' + String(e)); }
  };
    ...
    <aside className="w-64 border-r border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 flex flex-col overflow-hidden">
        ...
        {navItems.map((n) => {
          ...
          return (
            ...
            >
              ...
              {n.count > 0 && <span className="text-[10px] text-gray-400 dark:text-gray-500">{n.count}</span>}
            </button>
          );
        })}
      ...
      <div className="flex-1 overflow-auto p-2">
        ...
        {section === 'history' && (
          <HistoryList items={history}
            onClear={async () => { if (confirm('清空所有历史记录？')) { await clearHistory(); toast.success('已清空历史'); } }}
          />
        )}
        {section === 'favorites' && (
      ...
      <div className="px-2 py-2 border-t border-gray-200 dark:border-gray-800 space-y-1.5">
        <div className="relative">
          <button onClick={() => setEnvDropdown(!envDropdown)}
            ...
            <span className="truncate">{activeEnvId ? environments.find((e) => e.id === activeEnvId)?.name || '选择环境' : '选择环境'}</span>
          ...
          {envDropdown && (
            <div className="absolute bottom-full left-0 right-0 mb-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg z-20 max-h-40 overflow-auto">
              <button onClick={() => { setActiveEnv(null); setEnvDropdown(false); }}
                className={cn('w-full text-left px-2 py-1.5 text-xs', !activeEnvId && 'text-primary-600 bg-primary-50 dark:bg-primary-900/20')}>无环境</button>
              {environments.map((e) => (
                <button key={e.id} onClick={() => { setActiveEnv(e.id); setEnvDropdown(false); }}
              ...
              <div className="border-t border-gray-100 dark:border-gray-700">
                <button onClick={() => { setEditingEnv(null); setEnvDialog(true); setEnvDropdown(false); }}
                  className="w-full text-left px-2 py-1.5 text-xs text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/20">+ 新建环境</button>
              </div>
            </div>
          )}
        </div>
...
function TreeCollectionsList({
  items, savedRequests, expanded, onToggle, onNew, onRemove,
}: {
  items: Collection[];
          ...
          return (
            <div key={c.id}>
              ...
              <div className="group flex items-center rounded hover:bg-white dark:hover:bg-gray-800 cursor-pointer">
                <button onClick={() => onToggle(c.id)} className="p-0.5 text-gray-400 hover:text-gray-600 flex-shrink-0">
                ...
                <Link to={`/collection/${c.id}`}
                  ...
                  <span className="text-[10px] text-gray-400 dark:text-gray-500 flex-shrink-0">{children.length}</span>
                </Link>
                <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onRemove(c.id); }}
...
function RequestMiniItem({ item }: { item: Request }) {
  ...
  return (
    ...
    >
      <span className="text-[9px] font-bold text-gray-400 dark:text-gray-500 flex-shrink-0 w-8 text-right">{item.method}</span>
      <span className="text-xs text-gray-600 dark:text-gray-400 truncate flex-1">{item.name || item.url}</span>
      {statusMeta && (
        <span className={cn('px-1 h-3.5 inline-flex items-center text-[8px] rounded whitespace-nowrap flex-shrink-0', statusMeta.color)}>
          {statusMeta.label}
        </span>
      )}
    </div>
...
function FavoritesList({ items, onRemove }: { items: any[]; onRemove: (id: string) => void }) {
  ...
  return (
    <div className="space-y-1">
      {items.map((f) => (
        <RequestListItem key={f.id} item={f.request}
          right={
...
function HistoryList({ items, onClear }: { items: any[]; onClear: () => void }) {
  ...
  return (
    <div className="space-y-1">
      ...
      {items.map((h) => (
        <RequestListItem key={h.id} item={h.request}
          subtitle={`${h.response.status} · ${h.response.time_ms}ms`}
          right={<span className="text-[10px] text-gray-400 dark:text-gray-500 flex-shrink-0">{formatDate(h.created_at)}</span>}
        />
      ))}
    </div>
  );
...
/* ========== 通用请求列表项 ========== */
function RequestListItem({ item, subtitle, right }: { item: any; subtitle?: string; right?: React.ReactNode }) {
  const navigate = useNavigate();
  const location = useLocation();
  const statusMeta = REQUEST_STATUS_META[item.status as RequestStatus];
  return (
      ...
      <div className="flex items-center justify-between gap-1">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            ...
            {statusMeta && (
              <span className={cn('px-1 h-4 inline-flex items-center text-[9px] rounded whitespace-nowrap flex-shrink-0', statusMeta.color)}>
                {statusMeta.label}
              </span>
            )}
          ...
          {subtitle && <div className="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 truncate">{subtitle}</div>}
        </div>
        {right}
      </div>
    </div>
